...
function renderInvoicePdfBuffer(invoice, user, opts = {}) {
  // opts.payUrl  -> absolute URL the "Pay this invoice online" button links to.
  //                 When set, we render an accent-coloured button under the
  //                 totals so a card-pay or PayPal-style payment is one tap
  //                 away from the PDF. PDF anchors are clickable in every
  //                 mainstream viewer (Preview, Acrobat, mobile mail apps).
  return new Promise((resolve, reject) => {
    try {
      ...
      if (user.business_phone) fromMeta.push(String(user.business_phone));
      ...
      const recipientAddress = invoice.recipient_address
        ? String(invoice.recipient_address).trim()
        : '';
      const addrLines = recipientAddress ? recipientAddress.split('\n').filter(Boolean) : [];
      // Grow the block when a client address is present so the text doesn't
      // spill out of the grey panel.
      const blockH = addrLines.length ? Math.max(60, 44 + addrLines.length * 12) : 60;
      ...
      // June 2026: itemised invoices render one row per line item; the
      // single-description invoice keeps the original one-row layout.
      ...
      if (lineItems) {
        ...
        for (const it of lineItems) {
          ...
          const label = qty !== 1 ? `${it.description}  (${qty} x ${gbp(it.rate)})` : it.description;
          doc.fillColor(INK).font('Helvetica').fontSize(11)
            .text(label, LEFT + 4, cursor, { width: descColW - 8 });
          const h = Math.max(doc.y - cursor, 14);
          doc.fillColor(INK).font('Helvetica-Bold').fontSize(11)
            .text(gbp(it.amount), LEFT + descColW + 4, cursor, {
      ...
      // ── Pay this invoice online (#292) ────────────────────────────────────
      // Big accent-coloured button immediately under the totals so it's the
      // first thing the eye lands on after seeing what's owed. Skip when no
      // pay URL was provided (free users without a linked payment method).
      ...
      if (payUrl && /^https?:\/\//i.test(payUrl)) {
        ...
        const btnY = cursor;
        const ACCENT = '#F0A500';
        doc.save().fillColor(ACCENT).roundedRect(btnX, btnY, btnW, btnH, 8).fill().restore();
        doc.fillColor('#111111').font('Helvetica-Bold').fontSize(11)
          .text('Pay this invoice online', btnX, btnY + 12, {
            width: btnW, align: 'center', link: payUrl, underline: false,
          });
        doc.fillColor(HINT).font('Helvetica').fontSize(8)
          .text('or use the bank details below', btnX + btnW + 10, btnY + 14, {
            width: CONTENT_W - btnW - 10,
          });
        cursor += btnH + 14;
      }
      ...
      let renderedNotes = invoice.notes ? String(invoice.notes) : '';
      if (renderedNotes && user.bank_details) {
        const bank = String(user.bank_details).trim();
        renderedNotes = renderedNotes.split(bank).join('').replace(/\n{3,}/g, '\n\n').trim();
      }
      if (renderedNotes) {
        doc.fillColor(MUTED).font('Helvetica').fontSize(9)
          .text(renderedNotes, LEFT, cursor, {
