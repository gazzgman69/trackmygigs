...
// The sending domain falls back to this when MAIL_FROM/INVOICE_FROM are unset.
// Set MAIL_DOMAIN (or a full MAIL_FROM) to your verified Resend domain; the
// brand name (APP_NAME) is independent, so a rename never touches DNS.
const FALLBACK_DOMAIN = process.env.MAIL_DOMAIN || 'trackmygigs.app';
...
function defaultFrom() {
  return process.env.MAIL_FROM || `${APP_NAME} <no-reply@${FALLBACK_DOMAIN}>`;
}
...
function invoiceFromAddress() {
  ...
  const domain = domainOf(addressOf(defaultFrom())) || FALLBACK_DOMAIN;
  return `${local}@${domain}`;
}
...
async function sendEmail({ to, subject, html, text, fromName, fromAddress, replyTo, attachments }) {
  ...
  if (fromName) {
    const addr = fromAddress || addressOf(defaultFrom()) || `no-reply@${FALLBACK_DOMAIN}`;
