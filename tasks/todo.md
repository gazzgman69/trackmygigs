...
- [ ] FIX FIRST (blocker, with Gareth's OK): accepting a dep offer creates no gig for the
...
- [ ] AI stub: sim sets header X-Sim-AI=stub; /api/ai/* routes short-circuit to canned responses
      when the header + RELOAD_SECRET match, so flows that chain off AI run without burning usage
- [ ] Wipe previous [STRESS]/sim leftovers (offers, threads visible in Gareth's account)

## Phase 1 — Fleet setup (~150 users, full coverage beats raw count)

- [ ] Extend sim personas: every user gets a placeholder photo (i.pravatar.cc), bio, genres,
...
- [ ] Verify public pages for a sample: /epk/:slug, /share/:slug (plain, ?times=1, ?embed=1)

## Phase 2 — Dep + messaging (the heavy focus)

- [ ] Dep offers: send to specific people, broadcast, accept (verify gig lands in accepter's
      calendar + Offers counters + notifications), decline, snooze + reappear, nudge, cancel
      with replacement suggestion, expiry
- [ ] Chat: 1-to-1, group threads, gig threads, renames, leave/delete semantics, read receipts
      (1-to-1 ticks + group "Read by X of Y"), 40KB cap, optimistic-send under cold start
- [ ] Contextual sends both directions: gig share -> Add to my gigs, contact share -> save,
      setlist share -> Save to my repertoire (dedupe check), inbox previews
- [ ] Marketplace: post pick + FCFS + free-with-reason, radius matching, multi-instrument,
...
- [ ] Block/report: blocked users can't DM or see each other in discover

## Phase 3 — Money (invoices reconciled against money in/out)

- [ ] Invoice lifecycle x many users: draft -> edit -> send (mock) -> mark paid / chase /
      delete; line items; teaching bundles; numbering sequence; pay slugs resolve
- [ ] Receipts with photos (every add path), edit, delete, category filters
- [ ] Reconciliation: sum(confirmed gig fees) == /api/earnings income; invoice paid/unpaid/
...
- [ ] Push single gig, push-all, edit in TMG -> sync, pull Google-side edits, sync-now failure
      detail, nudge import/classify/dismiss, disconnect/reconnect
- [ ] Sheets: preview, import w/ column mapper, push edits back, pull, conflict resolver path
      (gigs.updated_at vs sheets_synced_at), disconnect
- [ ] All test artefacts tagged [TEST] and deleted afterwards

## Phase 5 — Everything-else sweep

- [ ] Gigs CRUD + wizard + quick-log parser edge cases; recurring teaching; CSV/Sheets import dedupe
- [ ] Blocked dates: single, weekly pattern incl. until-date, CSV upload, unblock, share page accuracy
- [ ] Setlists/songs/ChordPro; documents + expiry alerts; notifications centre + dismissals;
      settings/profile saves; auth (magic link request, logout, dev-login); premium gates as free user
- [ ] Concurrency torture: simultaneous FCFS takes, simultaneous picks, double-send guards

## Phase 6 — UX / navigation audit (browser, Gareth's priority)

- [ ] Walk core journeys as screenshots: first-run -> log gig -> invoice it -> get paid;
      receive dep -> accept -> see it in calendar; find musician -> chat -> book
- [ ] Tap-target sizes, dead ends, back-button consistency, panel stacking, empty states,
      number of taps per common action; note every friction point

## Phase 7 — Report

- [ ] tasks/FINDINGS.md: bugs (severity + repro), improvements, UX recommendations, prioritised
- [ ] Wipe all sim data, verify Gareth's account back to clean
- [ ] Roadmap docx entry + lessons
