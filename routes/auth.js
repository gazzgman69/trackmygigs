...
router.post('/delete-account', async (req, res) => {
  try {
    ...
    try {
      ...
      await run('DELETE FROM personal_events WHERE user_id = $1', [userId]);
      await run('DELETE FROM agencies WHERE user_id = $1', [userId]);
      await run('DELETE FROM gig_payments WHERE user_id = $1', [userId]);
