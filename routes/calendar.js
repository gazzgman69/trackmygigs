...
// ── Phase G+ OAuth refresh test (TEMPORARY) ─────────────────────────────────
// POST /api/calendar/test-expire-token?key=<RELOAD_SECRET>
// Forces google_token_expires_at to 1 minute in the past so the next call to
// getGoogleAuth() triggers the refresh_token flow. Returns a masked snapshot
// of the current token state so the test can compare before/after. Remove
// this endpoint once the refresh behavior has been verified.
router.post('/test-expire-token', async (req, res) => {
  try {
    const key = String(req.query.key || '');
    if (!process.env.RELOAD_SECRET || key !== process.env.RELOAD_SECRET) {
      return res.status(403).json({ error: 'forbidden' });
    }

    const before = await db.query(
      'SELECT google_access_token, google_token_expires_at FROM users WHERE id = $1',
      [req.user.id]
    );
    const beforeRow = before.rows[0] || {};
    const mask = (t) => (t ? `${String(t).slice(0, 10)}…${String(t).slice(-6)}` : null);

    const oneMinAgo = new Date(Date.now() - 60 * 1000);
    await db.query(
      'UPDATE users SET google_token_expires_at = $1 WHERE id = $2',
      [oneMinAgo, req.user.id]
    );

    res.json({
      ok: true,
      before: {
        access_token: mask(beforeRow.google_access_token),
        expires_at: beforeRow.google_token_expires_at,
      },
      forced_expires_at: oneMinAgo.toISOString(),
    });
  } catch (err) {
    console.error('test-expire-token error:', err);
    res.status(500).json({ error: err.message || 'failed' });
  }
});

// GET /api/calendar/test-token-state?key=<RELOAD_SECRET>
// Returns a masked snapshot of the current OAuth token state. Companion to
// test-expire-token — use before/after sync to verify refresh fired.
router.get('/test-token-state', async (req, res) => {
  try {
    const key = String(req.query.key || '');
    if (!process.env.RELOAD_SECRET || key !== process.env.RELOAD_SECRET) {
      return res.status(403).json({ error: 'forbidden' });
    ...
    const result = await db.query(
      'SELECT google_access_token, google_token_expires_at, google_refresh_token FROM users WHERE id = $1',
      [req.user.id]
    );
    const row = result.rows[0] || {};
    const mask = (t) => (t ? `${String(t).slice(0, 10)}…${String(t).slice(-6)}` : null);
    ...
    res.json({
      ok: true,
      access_token: mask(row.google_access_token),
      expires_at: row.google_token_expires_at,
      has_refresh_token: !!row.google_refresh_token,
      now: new Date().toISOString(),
    });
  } catch (err) {
    console.error('test-token-state error:', err);
    res.status(500).json({ error: err.message || 'failed' });
  }
});
